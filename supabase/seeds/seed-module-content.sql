-- Seed file for module_content table
-- Generated from modules-content-backup.ts
-- Contains all 66 steps from all 15 modules

INSERT INTO module_content (module_id, step_id, instructions, video_url) VALUES
  ('welcome', 'welcome-1', $md0$## Welcome to Disruptors Sales Infra
...
**Your next step:** Continue to the System Overview to see all 8 agents at a glance.$md0$, NULL),
  ('welcome', 'welcome-2', $md0$## Your 8 AI Agents — At a Glance
...
These agents don't operate in isolation. A new lead might come through the **Website Chatbot**, get followed up by the **Lead Follow-up** agent, receive reminders from the **Appointment Reminder** agent, and after service, get a review request from the **Review Request** agent — all automatically.$md0$, NULL),
  ('welcome', 'welcome-3', $md0$## Your Journey: From Setup to Scale
...
**The goal is simple:** You focus on delivering great service. We make sure your phone never stops ringing with qualified leads.$md0$, NULL),
  ('api-setup', 'api-setup-1', $md0$## You Own Everything We Build
...
**Bottom line:** We earn your business every month by delivering results, not by holding your systems hostage.$md0$, NULL),
  ('api-setup', 'api-setup-2', $md0$## GoHighLevel Setup
...
- **Choose Option B** if you want to keep things simple and save on the $297/month subscription$md0$, NULL),
  ('api-setup', 'api-setup-3', $md0$## OpenAI API Key Setup
...
**You pay OpenAI directly** — complete transparency on AI costs, no markup from us.$md0$, NULL),
  ('api-setup', 'api-setup-4', $md0$## Retell.ai Voice Agent Setup
...
Retell charges per minute of call time. Most businesses spend **$15-40/month** depending on call volume. You'll see exact usage in your Retell dashboard.$md0$, NULL),
  ('api-setup', 'api-setup-5', $md0$## Connection Verification Checklist
...
**If you're stuck on any step, use the Support tab to chat with our AI assistant or submit a feedback request.**$md0$, NULL),
  ('twilio-setup', 'twilio-setup-1', $md0$## Twilio Setup — Overview
...
Review the setup checklist, then schedule a call with your CSM to complete the setup together.$md0$, NULL),
  ('twilio-setup', 'twilio-setup-2', $md0$## Create Your Twilio Account
...
After creating your account, enter your login and API credentials in the form below.$md0$, NULL),
  ('twilio-setup', 'twilio-setup-3', $md0$## Schedule Your Twilio Setup Call
...
- All credentials are saved in this platform$md0$, NULL),
  ('voice-receptionist', 'voice-receptionist-1', $md0$## AI Voice Receptionist — Overview & ROI
...
- Cost of AI receptionist: **~$50/month**$md0$, NULL),
  ('voice-receptionist', 'voice-receptionist-2', $md0$## How the Voice Receptionist Works
...
In the next step, you'll adjust the AI's greeting, qualification questions, and knowledge base to match your specific business.$md0$, NULL),
  ('voice-receptionist', 'voice-receptionist-3', $md0$## Customize Your AI Voice Receptionist
...
Once you've filled everything out, hit Save. For this one, just pretend you're training a brand new receptionist. Tell them exactly how you want calls handled.$md0$, NULL),
  ('voice-receptionist', 'voice-receptionist-4', $md0$## Test Your Voice Receptionist With Your CSM
...
**If anything needs adjustment,** note it in the feedback form on the next step. Our team will fine-tune the prompt and redeploy.$md0$, NULL),
  ('voice-receptionist', 'voice-receptionist-5', $md0$## Review & Complete — AI Voice Receptionist
...
**No request is too small.** We want this to sound exactly like your business.$md0$, NULL),
  ('db-reactivation', 'db-reactivation-1', $md0$## Database Reactivation — Overview & ROI
...
The best part? These are people who already know your business. The AI just re-opens the conversation.$md0$, NULL),
  ('db-reactivation', 'db-reactivation-2', $md0$## How Database Reactivation Works
...
- **Duplicate prevention** — contacts already in an active campaign are excluded$md0$, NULL),
  ('db-reactivation', 'db-reactivation-3', $md0$## Customize Your Database Reactivation Agent
...
Once everything is filled out, hit Save and our team will build it out for you.$md0$, NULL),
  ('db-reactivation', 'db-reactivation-4', $md0$## Test Your Database Reactivation With Your CSM
...
Use the button below to book a time with your CSM. Calls typically take 20-30 minutes.$md0$, NULL),
  ('db-reactivation', 'db-reactivation-5', $md0$## Review & Complete — Database Reactivation
...
**Submit any final feedback below before marking complete.**$md0$, NULL),
  ('lead-followup', 'lead-followup-1', $md0$## Lead Follow-up — Overview & ROI
...
- Your AI follows up **7-10 times** over 14 days automatically$md0$, NULL),
  ('lead-followup', 'lead-followup-2', $md0$## The Lead Follow-up Sequence
...
If the lead replies, the AI picks up the conversation naturally and works toward booking an appointment. The sequence pauses automatically once a conversation starts.$md0$, NULL),
  ('lead-followup', 'lead-followup-3', $md0$## Customize Your Follow-Up Agent
...
Once everything is filled out, hit Save and our team will build it out for you.$md0$, NULL),
  ('lead-followup', 'lead-followup-4', $md0$## Test Your Lead Follow-up With Your CSM
...
Use the button below to book a time with your CSM. Calls typically take 20-30 minutes.$md0$, NULL),
  ('lead-followup', 'lead-followup-5', $md0$## Review & Complete — Lead Follow-up
...
The first week of data is the most valuable. After launch, review the AI conversations and let us know if any responses need adjusting. Most clients see optimal performance after 1-2 rounds of tuning.$md0$, NULL),
  ('appointment-reminders', 'appointment-reminders-1', $md0$## Appointment Reminders — Overview & ROI
...
- If you do 80 appointments/month at $150 average: that's **$1,500-3,000/month** in saved revenue$md0$, NULL),
  ('appointment-reminders', 'appointment-reminders-2', $md0$## How Appointment Reminders Work
...
All reminders sync with your GoHighLevel calendar. Any changes to the appointment automatically update the reminder schedule.$md0$, NULL),
  ('appointment-reminders', 'appointment-reminders-3', $md0$## Customize Your Appointment Setter Agent
...
Once everything is filled out, hit Save and our team will build it out for you.$md0$, NULL),
  ('appointment-reminders', 'appointment-reminders-4', $md0$## Test Your Appointment Reminders With Your CSM
...
Use the button below to book a time with your CSM. Calls typically take 15-20 minutes.$md0$, NULL),
  ('appointment-reminders', 'appointment-reminders-5', $md0$## Review & Complete — Appointment Reminders
...
**Submit any final feedback below, then mark this module complete.**$md0$, NULL),
  ('quote-followup', 'quote-followup-1', $md0$## Quote Follow-up — Overview & ROI
...
- Your AI never forgets, never gets busy, and always follows up on schedule$md0$, NULL),
  ('quote-followup', 'quote-followup-2', $md0$## How Quote Follow-up Works
...
- If they're **ready to move forward**, the AI routes them to you for closing$md0$, NULL),
  ('quote-followup', 'quote-followup-3', $md0$## Customize Your Quote Follow-up
...
- Stop follow-up after quote expires$md0$, NULL),
  ('quote-followup', 'quote-followup-4', $md0$## Test Your Quote Follow-up With Your CSM
...
Use the button below to book a time with your CSM. Calls typically take 20-30 minutes.$md0$, NULL),
  ('quote-followup', 'quote-followup-5', $md0$## Review & Complete — Quote Follow-up
...
**Submit any final feedback below, then mark this module complete.**$md0$, NULL),
  ('review-request', 'review-request-1', $md0$## Review Request — Overview & ROI
...
- Most happy clients WILL leave a review — they just need to be asked$md0$, NULL),
  ('review-request', 'review-request-2', $md0$## How Review Request Works
...
- **Multi-platform** — can direct to Google, Yelp, Facebook, or any custom URL$md0$, NULL),
  ('review-request', 'review-request-3', $md0$## Customize Your Review Requests
...
- Any standard recovery offers? (discount, redo, refund policy)$md0$, NULL),
  ('review-request', 'review-request-4', $md0$## Test Your Review Request System With Your CSM
...
Use the button below to book a time with your CSM. Calls typically take 15-20 minutes.$md0$, NULL),
  ('review-request', 'review-request-5', $md0$## Review & Complete — Review Request
...
**Your online reputation is about to grow on autopilot. Submit any final feedback below.**$md0$, NULL),
  ('website-chatbot', 'website-chatbot-1', $md0$## Website Chatbot — Overview & ROI
...
- The chatbot works **24/7** — capturing leads even at 2am$md0$, NULL),
  ('website-chatbot', 'website-chatbot-2', $md0$## How the Website Chatbot Works
...
- **Contact page** → offers to book right there$md0$, NULL),
  ('website-chatbot', 'website-chatbot-3', $md0$## Customize Your Engagement Agent
...
Once everything is filled out, hit Save and our team will build it out for you.$md0$, NULL),
  ('website-chatbot', 'website-chatbot-4', $md0$## Test Your Website Chatbot With Your CSM
...
Use the button below to book a time with your CSM. Calls typically take 15-20 minutes.$md0$, NULL),
  ('website-chatbot', 'website-chatbot-5', $md0$## Review & Complete — Website Chatbot
...
**Submit any final feedback below, then mark this module complete.**$md0$, NULL),
  ('a2p-registration', 'a2p-1', $md0$## A2P 10DLC Registration — Overview
...
- No recurring fees for A2P itself$md0$, NULL),
  ('a2p-registration', 'a2p-2', $md0$## Business Information for A2P Registration
...
Fill in the credential fields below with your business information. Your CSM will use these to submit your registration.$md0$, NULL),
  ('a2p-registration', 'a2p-3', $md0$## Campaign Registration
...
You don't need to fill out carrier forms — your CSM submits everything using the business information you provided in the previous step.$md0$, NULL),
  ('a2p-registration', 'a2p-4', $md0$## Registration Status
...
Contact your CSM or check the Support tab. We monitor registrations daily and will proactively reach out if there are any issues.$md0$, NULL),
  ('knowledge-base', 'kb-1', $md0$## Knowledge Base — The Brain of Your AI
...
AI agents with detailed knowledge bases outperform generic ones by 3-5x in customer satisfaction. Take your time with this module.$md0$, NULL),
  ('knowledge-base', 'kb-2', $md0$## Services & Pricing Questionnaire
...
Enter your services and pricing details in the text area below. Be as detailed as possible — this directly affects how well your AI handles inquiries.$md0$, NULL),
  ('knowledge-base', 'kb-3', $md0$## FAQs & Business Details
...
Type or paste your FAQs and business details in the form below. Don't worry about perfect formatting — we'll organize it for you.$md0$, NULL),
  ('knowledge-base', 'kb-4', $md0$## Review & Finalize Your Knowledge Base
...
| Campaigns | Manage text campaigns |$md0$, NULL),
  ('management', 'management-2', $md0$## Demo to Live — Transition Guide
...
For the first 2 weeks after go-live, your CSM monitors everything closely and will proactively reach out if they see any issues.$md0$, NULL),
  ('management', 'management-3', $md0$## Ongoing Optimization
...
- Scaling conversations as your volume grows$md0$, NULL),
  ('management', 'management-4', $md0$## Congratulations — You're Fully Onboarded!
...
Thank you for choosing Disruptors Media. We're committed to making your business the most responsive, professional, and automated operation in your market. Let's grow! 🚀$md0$, NULL),
  ('prompt-playground', 'prompt-playground-1', $md0$## Prompt Playground — What It Is
...
You don't need to be an AI expert. The Playground uses plain English — just describe how you want the AI to behave, and test it. Our team is here to help if you get stuck.$md0$, NULL),
  ('prompt-playground', 'prompt-playground-2', $md0$## How to Use the Prompt Playground
...
- **Test in batches** — run 5-10 different test messages to cover various scenarios$md0$, NULL),
  ('prompt-playground', 'prompt-playground-3', $md0$## Try It Yourself
...
| Wrong info | Update the knowledge section with correct details |$md0$, NULL),
  ('prompt-playground', 'prompt-playground-4', $md0$## Save & Deploy
...
**Congratulations — you've completed the Prompt Playground module!**$md0$, NULL)
