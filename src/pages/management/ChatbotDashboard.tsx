...
export default function ChatbotDashboard() {
  ...
  const { data, loading } = useAnalytics('chat')
  ...
  return (
    ...
    <div className="space-y-6">
      ...
      {hasData ? (
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <StatsCard
            ...
            value={getMetricValue(data, 'user_questions')}
            icon={<MessageSquare className="h-5 w-5" />}
            iconBg="bg-blue-100 text-blue-600"
          />
          <StatsCard
            label="Bot Responses"
            value={getMetricValue(data, 'bot_messages')}
            icon={<Bot className="h-5 w-5" />}
            iconBg="bg-green-100 text-green-600"
          />
          <StatsCard
            label="Active Conversations"
            value={getMetricValue(data, 'active_conversations')}
            icon={<Users className="h-5 w-5" />}
            iconBg="bg-purple-100 text-purple-600"
          />
          <StatsCard
            label="Response Rate"
            value={`${getMetricValue(data, 'response_rate')}%`}
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Analytics will appear once your chatbot starts handling
              conversations.
            </p>
          </CardContent>
        </Card>
      ...
      <div className="flex flex-wrap gap-3">
        <Button
          variant="outline"
          onClick={() => navigate('../chatbot/chat-with-ai')}
        >
